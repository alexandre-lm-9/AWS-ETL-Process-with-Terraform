resource "aws_s3_bucket_object" "raw-data-code" {
  ...
  source = "../s3_upload.py"           #path do arquivo a ser subido
  etag   = md5("../s3_upload_raw-data.py")  # se não houver mudanca, nao sobe o dado novamente
}
...
resource "aws_s3_bucket_object" "pyspark-code" {
  ...
  acl = "private" # Acesso a lista
  source = "pyspark_job.py" #path do arquivo a ser subido
  etag = md5("../pyspark_job.py") # se não houver mudanca, nao sobe o dado novamente
}
